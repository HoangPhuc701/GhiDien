package com.example.appghidien.Model;

public class HoaDonBangGhiDien {
    private String MaBangGhiDien;
    private String MaHoaDon;
    private String ThangGhi;
    private String ChiSoCu;
    private String ChiSoMoi;
    private String ChiSoTieuThu;
    private String TienDien;
    private String TienThue;
    private String ThanhTien;
    private String TrangThaiThanhToan;
    private String MaDongHoDien;
    private String SoNha;
    private String TenDuong;
    private String TenChuHo;

    public HoaDonBangGhiDien() {
    }

    public HoaDonBangGhiDien(String maBangGhiDien, String maHoaDon, String thangGhi, String chiSoCu, String chiSoMoi, String chiSoTieuThu, String tienDien, String tienThue, String thanhTien, String trangThaiThanhToan, String maDongHoDien, String soNha, String tenDuong) {
        MaBangGhiDien = maBangGhiDien;
        MaHoaDon = maHoaDon;
        ThangGhi = thangGhi;
        ChiSoCu = chiSoCu;
        ChiSoMoi = chiSoMoi;
        ChiSoTieuThu = chiSoTieuThu;
        TienDien = tienDien;
        TienThue = tienThue;
        ThanhTien = thanhTien;
        TrangThaiThanhToan = trangThaiThanhToan;
        MaDongHoDien = maDongHoDien;
        SoNha = soNha;
        TenDuong = tenDuong;
    }

    public String getMaBangGhiDien() {
        return MaBangGhiDien;
    }

    public void setMaBangGhiDien(String maBangGhiDien) {
        MaBangGhiDien = maBangGhiDien;
    }

    public String getMaHoaDon() {
        return MaHoaDon;
    }

    public void setMaHoaDon(String maHoaDon) {
        MaHoaDon = maHoaDon;
    }

    public String getThangGhi() {
        return ThangGhi;
    }

    public void setThangGhi(String thangGhi) {
        ThangGhi = thangGhi;
    }

    public String getChiSoCu() {
        return ChiSoCu;
    }

    public void setChiSoCu(String chiSoCu) {
        ChiSoCu = chiSoCu;
    }

    public String getChiSoMoi() {
        return ChiSoMoi;
    }

    public void setChiSoMoi(String chiSoMoi) {
        ChiSoMoi = chiSoMoi;
    }

    public String getChiSoTieuThu() {
        return ChiSoTieuThu;
    }

    public void setChiSoTieuThu(String chiSoTieuThu) {
        ChiSoTieuThu = chiSoTieuThu;
    }

    public String getTienDien() {
        return TienDien;
    }

    public void setTienDien(String tienDien) {
        TienDien = tienDien;
    }

    public String getTienThue() {
        return TienThue;
    }

    public void setTienThue(String tienThue) {
        TienThue = tienThue;
    }

    public String getThanhTien() {
        return ThanhTien;
    }

    public void setThanhTien(String thanhTien) {
        ThanhTien = thanhTien;
    }

    public String getTrangThaiThanhToan() {
        return TrangThaiThanhToan;
    }

    public void setTrangThaiThanhToan(String trangThaiThanhToan) {
        TrangThaiThanhToan = trangThaiThanhToan;
    }

    public String getMaDongHoDien() {
        return MaDongHoDien;
    }

    public void setMaDongHoDien(String maDongHoDien) {
        MaDongHoDien = maDongHoDien;
    }

    public String getSoNha() {
        return SoNha;
    }

    public void setSoNha(String soNha) {
        SoNha = soNha;
    }

    public String getTenDuong() {
        return TenDuong;
    }

    public void setTenDuong(String tenDuong) {
        TenDuong = tenDuong;
    }

    public String getTenChuHo() {
        return TenChuHo;
    }

    public void setTenChuHo(String tenChuHo) {
        TenChuHo = tenChuHo;
    }
}
